...
export default function Page() {
    ...
    async function getUserInfo() {
      try {
        const response = await fetch(
          "https://api.github.com/users/pablokaliel/repos?per_page=100"
        );
        if (!response.ok) {
  ...
  const [gridColumns, setGridColumns] = useState<"grid-cols-3" | "grid-cols-5">(
    "grid-cols-3"
  );
    ...
    function updateGridColumns() {
      ...
      if (container) {
        ...
        const newGridColumns =
          containerWidth < 600 ? "grid-cols-3" : "grid-cols-5";
    ...
    <main className="flex px-2 flex-col h-full bg-[#272727]">
      <div ref={gridContainerRef} className="grid-container">
        {loading ? (
          ...
          <div>
            <div
              className={`grid ${gridColumns} pb-8 place-content-center gap-2`}
            >
